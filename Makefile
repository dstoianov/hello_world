...
lint-py:
	...
	# Requires flake8
	$(WORKON_HOME)/hello_world/bin/flake8 .
...
setup:
	virtualenv -p `which python3.5` $(WORKON_HOME)/hello_world
	$(WORKON_HOME)/hello_world/bin/pip install -U pip wheel
	$(WORKON_HOME)/hello_world/bin/pip install -Ur requirements/dev.txt
	$(WORKON_HOME)/hello_world/bin/pip freeze
	npm install
	npm update
	cp hello_world/settings/local.example.py hello_world/settings/local.py
	echo "DJANGO_SETTINGS_MODULE=hello_world.settings.local" > .env
	createdb -E UTF-8 hello_world
	$(WORKON_HOME)/hello_world/bin/python manage.py migrate
	if [ -e project.travis.yml ] ; then mv project.travis.yml .travis.yml; fi
	@echo
	@echo "The hello_world project is now setup on your machine."
	@echo "Run the following commands to activate the virtual environment and run the"
	@echo "development server:"
	@echo
	@echo "	workon hello_world"
	@echo "	npm run dev"

update:
	$(WORKON_HOME)/hello_world/bin/pip install -U -r requirements/dev.txt
	npm install
	npm update
